  ...
  var redmine = {
    ...
    start: function(config) {
      ...
      if( config.query ) {
        ...
        jsonKey = "issues";
        this.url = config.host + 'issues.json?fixed_version_id=' + config.version + '&limit=100&key=' + config.key;
      }
      ...
      this.download(0, jsonKey, (config.cb?config.cb:this.output), function(err){
          throw "[ERROR] " + err;
      });
    }
  };
